...
from app.generator import get_base_for_station, _get_profile_std

# Default fallback scale factors for Z-scores
DEFAULT_SCALES = {
    'temperature': 1.4,
    'pressure': 1.8,
    'humidity': 2.5
...
def extract_features_from_window(
    ...
    t_diff, t_std, t_trend = calc_param_features(temps, base['temp'], scales.get('temperature', 1.4))
    p_diff, p_std, p_trend = calc_param_features(press, base['pressure'], scales.get('pressure', 1.8))
    h_diff, h_std, h_trend = calc_param_features(hums, base['humidity'], scales.get('humidity', 2.5))
...
class AnomalyDetector:
    ...
    def evaluate_reading(
        ...
        base = get_base_for_station(station_id)

        # Scales per parameter derived from real standard deviations, scaled appropriately
        stds = _get_profile_std(station_id)
        scales = {
            'temperature': max(1.0, stds['temp'] * 0.35),
            'pressure': max(1.0, stds['pressure'] * 0.8),
            'humidity': max(1.5, stds['humidity'] * 0.25)
        }
        ...
        feats = extract_features_from_window(window, base, scales)
